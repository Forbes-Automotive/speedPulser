...
function initControls() {
  ...
  const configInputs = ['hasNeedleSweep', 'sweepSpeed', 'motorCalSelection', 'maxSpeed', 'maxFreqHall', 'speedOffsetPositive', 'speedOffset', 'convertToMPH', 'averageFilter', 'reverseDirection', 'feedbackEnable', 'feedbackMaxFreq', 'pidKp', 'pidKi', 'pidKd'];
...
async function fetchSettings() {
  try {
    ...
    document.getElementById('feedbackMaxFreq').value = data.feedbackMaxFreq || 200;
    document.getElementById('feedbackMaxFreq-display').textContent = data.feedbackMaxFreq || 200;
    document.getElementById('pidKp').value = data.pidKp ?? 0.5;
    document.getElementById('pidKp-display').textContent = data.pidKp ?? 0.5;
    document.getElementById('pidKi').value = data.pidKi ?? 0.1;
    document.getElementById('pidKi-display').textContent = data.pidKi ?? 0.1;
...
async function fetchStatus() {
  ...
  try {
    ...
    if (measuredEl) measuredEl.textContent = data.measuredSpeed ?? '--';
    const pidEl = document.getElementById('pidCorrection');
    if (pidEl && data.pidCorrection !== undefined) {
      const n = Number(data.pidCorrection);
      pidEl.textContent = (n > 0 ? '+' : '') + n;
    }
