...
bool reverseDirection = false;        // false = forward (pinDirection HIGH), true = reverse (LOW)
bool feedbackEnable = false;          // enable PID speed feedback trim using GPIO4
float pidKp = 0.5f;                   // PID proportional gain
float pidKi = 0.1f;                   // PID integral gain
float pidKd = 0.0f;                   // PID derivative gain
uint16_t feedbackMaxFreq = 200;       // feedback Hz that equals maxSpeed (motor feedback scaling)
