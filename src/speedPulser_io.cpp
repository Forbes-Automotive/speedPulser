...
void basicInit()
{
    ...
    DEBUG_PRINTLN("Setting up Direction Output...");
    pinMode(pinDirection, OUTPUT);
    applyDirection();  // forward = HIGH, reverse = LOW
    DEBUG_PRINTLN("Set up Direction Output!");

    DEBUG_PRINTLN("Setting up Feedback Input (GPIO4)...");
    ...
    DEBUG_PRINTLN("Set up Feedback Input!");

    DEBUG_PRINTLN("Initialised SpeedPulser!");
}

// Drive the direction pin from the reverseDirection flag.
// LOW reverses the motor; HIGH = forward.
void applyDirection()
{
    digitalWrite(pinDirection, reverseDirection ? LOW : HIGH);
}
